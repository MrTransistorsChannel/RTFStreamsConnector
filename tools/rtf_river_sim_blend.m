% rtf_river_sim.m            - WINNER file (connector port, A1/A2): per-chunk
%                              candidate admission (ChunkRiverCache.java) +
%                              per-block winner flow (RTFFlowProvider.java).
...
% The game command /rtfconnector dumprivers writes a JSON dump of a region's
% rivers; copy it into this folder (tools/) and the sim will load the NEWEST
% dump instead of the hard-coded rivers below (hardcodedNetworks()).
networks = load_network_dump(fileparts(mfilename('fullpath')));
if isempty(networks)
    networks = hardcodedNetworks();
    fprintf('load_network_dump: no JSON dump found in the sim folder, using HARD-CODED rivers.\n');
else
    fprintf('load_network_dump: loaded %d root network(s) from JSON dump.\n', numel(networks));
end
...
% EXAMPLES: the sim is now connector-based (ChunkRiverCache.java admission +
% RTFFlowProvider.java winner flow, A1/A2). The old gradient methods
% (flowGradientNormal, flowSideGradient, flowSignedDistGradient) are KEPT defined
% below for reference but are no longer probed by default. To re-probe them,
% uncomment their probeFlow lines:
...
for i = 1:size(jungrid, 1)
    ...
    cands = chunkCandidateChains(networks, floor(x/16), floor(z/16), 1.0);
    line = sprintf('CHECK | junction (%5d,%5d) |', x, z);
    for c = 1:numel(cands)
        chain = cands{c}; L = chain(end);
        ...
        d2 = w*w + lOver*lOver;
        gate2 = min(scaledSize2(l/L.river.length, L, L.banksWidth^2, 1.25^2)/1.0, L.valleyWidth^2);
        tag = 'COVER'; if d2 > gate2, tag = '--'; end
...
sweeps = { 'main t=0.05', 2850.1, -7168.9,  0.9089, 0.4171; ...
           'main t=0.50', 1231.0, -3641.0,  0.9089, 0.4171; ...
           'child s=0.5', 3143.3, -5371.1,  0.5438, 0.8392 };
for i = 1:size(sweeps, 1)
    sname = sweeps{i, 1}; px = sweeps{i, 2}; pz = sweeps{i, 3}; nx = sweeps{i, 4}; nz = sweeps{i, 5};
    bestD2 = inf; best = [];
    for s = -350:8:350
        x = round(px + s*nx); z = round(pz + s*nz);
        [dx, dz] = flowConnectorWinner(networks, x, z, 1.0, checkCache);
        ...
        for c = 1:numel(cands)
            ...
            if d2 <= gate2 && d2 < bestD2
                bestD2 = d2; best = [x, z, dx, dz, L.river.x1, L.river.z1];
            end
        end
    end
    if isempty(best)
        fprintf('CHECK | %-12s | NO COVERED POINT in the strip\n', sname);
    else
...
function networks = hardcodedNetworks()
...
%% Connector-port: per-chunk admission + per-block winner flow
% Ports of ChunkRiverCache.java (findVertexCandidates/findCandidatesRecursively,
% A1) and RTFFlowProvider.java (flowRadAt, A2). Differences from the game:
%  - the regional DOMAIN WARP (Rivermap riverWarp Domain) is NOT simulated
%    (identity): the sim folds per-node RiverWarp offsets only, like the old
%    helpers. Probe points = the vertex blocks themselves.
%  - mouthModifier defaults to 1.0: the sim has no continent edge, and the game
%    formula map(edge, 0, 0.5, 0.5)^2 is 1.0 for edge == 1.
%  - caching (the spec's "except for caching" part): candCache is keyed
%    GEOMETRICALLY by chunk coords (cx*1e6+cz) - never by runtime object
%    identity (Rivermap networks get recreated on the ~5-min TTL).

function cands = chunkCandidateChains(networks, cx, cz, mouthModifier)
    % A1 - per-chunk candidate admission: 4 vertex probes at the chunk's corner
    % blocks (vx*16, vz*16); root AABB gate via treeBounds; depth-first
    % admission walk with the nodeX/nodeZ sibling rule; dedup by the leaf's
    % geometric key. Returns a cell array of chains (each chain = 1xk struct
    % array root->...->admitted node, copied by value), or {}.
    if nargin < 4 || isempty(mouthModifier), mouthModifier = 1.0; end
    cands = {}; keys = {};
    for vx = cx:cx+1
        for vz = cz:cz+1
            px = vx*16; pz = vz*16;                    % corner block (vertex probe)
            roots = networks(1:0);                     % empty struct array (growing [] would stay double)
            for i = 1:numel(networks)
                [bxmn, bxmx, bzmn, bzmx] = treeBounds(networks(i));
                if px >= bxmn && px <= bxmx && pz >= bzmn && pz <= bzmx
                    roots(end+1) = networks(i);
                end
            end
            for i = 1:numel(roots)
                [cands, keys] = admissionWalk(roots(i), px, pz, 0, mouthModifier, networks(1:0), cands, keys);
            end
        end
    end
end

function [cands, keys] = admissionWalk(node, ex, ez, depth, mouthModifier, chain, cands, keys)
    % findCandidatesRecursively (ChunkRiverCache.java:100-141): every SIBLING
    % enters from the SAME point (ex,ez); the node's own fold is applied to a
    % COPY (nx,nz), and only children get the FOLDED point as their entering
    % point - one sibling's fold never changes the next sibling's start.
    if depth >= 4, return; end                       % MAX_CHAIN_DEPTH = 4
    rv = node.river;
    nx = ex; nz = ez;
    [odx, odz] = shiftAt(rv, node.warp, nx, nz);     % warp.getOffset (shiftAt has the t in [0,1] test built in)
    nx = nx + odx; nz = nz + odz;
    % admission distance of the FOLDED point (== getDistSq with the UNclamped t)
    w = (nx - rv.x1)*rv.normX + (nz - rv.z1)*rv.normZ;
    l = (nx - rv.x1)*rv.ndx   + (nz - rv.z1)*rv.ndz;
    lOver = l - min(max(l, 0), rv.length);
    d2 = w*w + lOver*lOver;
    % admission gate (ChunkRiverCache.java:132-136): banksWidth2 =
    % min(bw^2/mouthModifier, valleyWidth^2); radius2 = max(64+banksWidth2, 128)
    banksWidth2 = min(node.banksWidth^2 / mouthModifier, node.valleyWidth^2);
    radius2 = max(64 + banksWidth2, 128);
    if d2 <= radius2
        leaf = node;
        key = [double(leaf.warp.seed), round(leaf.river.x1*1e4), round(leaf.river.z1*1e4), ...
               round(leaf.river.x2*1e4), round(leaf.river.z2*1e4)];
        if isempty(keys) || ~any(cellfun(@(k) isequal(k, key), keys))
            keys{end+1} = key;                       % geometric dedup across the 4 vertex probes
            cands{end+1} = [chain, node];            % chain copy root->...->node
        end
    end
    for c = 1:numel(node.children)
        if ~isempty(node.children(c).river)
            [cands, keys] = admissionWalk(node.children(c), nx, nz, depth+1, mouthModifier, [chain, node], cands, keys);
        end
    end
end

function cands = candidateList(networks, x, z, mouthModifier, candCache)
    % Candidate chains of the chunk containing block (x,z), lazily built once
    % per chunk into candCache (containers.Map, GEOMETRIC key cx*1e6+cz - the
    % "except for caching" part; never identity-keyed).
    cx = floor(x/16); cz = floor(z/16);
    if isempty(candCache)
        cands = chunkCandidateChains(networks, cx, cz, mouthModifier);
        return;
    end
    key = cx*1e6 + cz;
    if candCache.isKey(key)
        cands = candCache(key);
    else
        cands = chunkCandidateChains(networks, cx, cz, mouthModifier);
        candCache(key) = cands;
...
function [fx, fz] = foldChainAt(chain, x, z)
    % foldWarpChain (RTFFlowProvider.java:121-137) minus the domain warp: fold
    % (x,z) through EVERY chain node's own warp while its section t is in [0,1].
    fx = x; fz = z;
    for i = 1:numel(chain)
        rv = chain(i).river;
        t = distanceOnLine(fx, fz, rv);
        if t >= 0 && t <= 1
            [odx, odz] = shiftAt(rv, chain(i).warp, fx, fz);
            fx = fx + odx; fz = fz + odz;
        end
    end
end

function w = signedLatDistChain(chain, x, z)
    % getDistanceToCandidate (RTFFlowProvider.java:100-118): signed lateral
    % distance of the chain-folded point onto the LEAF's normal (no clamp).
    [fx, fz] = foldChainAt(chain, x, z);
    L = chain(end);
    w = (fx - L.river.x1)*L.river.normX + (fz - L.river.z1)*L.river.normZ;
end

function [dx, dz] = chainDirAt(chain, x, z, h)
    % A2 DIRECTION (RTFFlowProvider.java:79-88): central-difference gradient
    % (h = 1 block, 4 live folds) of the chain's signed lateral distance,
    % rotated 90 deg; axis fallback when the gradient is ~zero.
    L = chain(end);
    gx = (signedLatDistChain(chain, x+h, z) - signedLatDistChain(chain, x-h, z)) / (2*h);
    gz = (signedLatDistChain(chain, x, z+h) - signedLatDistChain(chain, x, z-h)) / (2*h);
    mag = sqrt(gx*gx + gz*gz);
    if mag <= 1e-5
        dx = L.river.ndx; dz = L.river.ndz;
    else
        dx = -gz/mag; dz = gx/mag;
    end
end

function [dx, dz] = flowConnectorWinner(networks, x, z, mouthModifier, candCache)
    % A2 - per-block winner flow (RTFFlowProvider.flowRadAt): fold every
    % candidate chain at (x,z), clamped d2 vs the leaf's scaled-bank gate
    % min(scaledBW2/mouthModifier, valleyWidth^2); winner = covering candidate
    % with the SMALLEST d2 (ties: first found). NaN/NaN when none covers.
    if nargin < 4 || isempty(mouthModifier), mouthModifier = 1.0; end
...
function probeFlowChunked(networks, step, flowFunc, color, arrowLen, mouthModifier)
    % probeFlow twin for the connector flow functions: builds a per-chunk
    % candidate cache (containers.Map, key cx*1e6+cz -> candidate cell array)
    % lazily over the raster chunks, so the A1 admission walk runs ONCE per
    % chunk, then probes every raster block with
    %   [dx, dz] = flowFunc(networks, x, z, mouthModifier, candCache)
    % Everything else identical to probeFlow (same raster, ONE quiver call,
    % equal arrow lengths).
    if nargin < 4, color = 'k'; end
    if nargin < 5, arrowLen = 20; end
    if nargin < 6 || isempty(mouthModifier), mouthModifier = 1.0; end
    candCache = containers.Map('KeyType', 'double', 'ValueType', 'any');
    [xr, zr] = networkRaster(networks, 1600, step);
    ax = gca; hold(ax, 'on');
    X = []; Y = []; U = []; V = [];
    for iz = 1:numel(zr)
        z = zr(iz);
        for ix = 1:numel(xr)
            [dx, dz] = flowFunc(networks, xr(ix), z, mouthModifier, candCache);
            if isnan(dx) || isnan(dz), continue; end
            L = hypot(dx, dz);
            if L <= 0, continue; end
            X(end+1) = xr(ix); Y(end+1) = z;
